...
inline bool is_multifile(const TorrentFile& t) {
    return t.files.has_value();
}

inline std::vector<std::string> generate_pieces(const std::string& raw) {
    std::vector<std::string> pieces;
...
inline uint64_t calculate_total_length(const TorrentFile& t) {
...
inline uint64_t piece_size(const TorrentFile& t, size_t piece_index) {
    if (piece_index == t.pieces.size() - 1)
        return t.total_length - (piece_index * t.piece_length);
    return t.piece_length;
}

inline TorrentFile parse_torrent(const std::string& data) {
    TorrentFile torrent;
...
inline void print_torrent(const TorrentFile& t, bool flag = false) {
