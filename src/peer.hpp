...
inline std::vector<uint8_t> build_handshake(const TorrentFile& torrent,
...
inline std::vector<PeerSocket> tcp_connect_peers(const std::vector<Peer>& peers) {
    ...
    for (const auto& peer : peers) {
        ...
        connect(fd, res->ai_addr, res->ai_addrlen);   // always returns EINPROGRESS
        freeaddrinfo(res);
...
inline std::vector<PeerConnection> handshake_peers(const std::vector<PeerSocket>& sockets,
    ...
    while (!pending.empty()) {
        ...
        for (size_t i = 0; i < pending.size(); i++) {
            ...
            if (!(pfds[i].revents & POLLIN)) {
                still_pending.push_back(p);          // nothing yet
                continue;
            }
            ...
            if (p.received < 68) {
                still_pending.push_back(p);          // partial
                continue;
            }
    ...
    for (const auto& p : pending)   // timed out
        close(p.sockfd);
